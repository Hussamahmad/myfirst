...
class SearchResultsView(generic.ListView):
    ...
    def get_queryset(self):
        if 'name' in self.request.GET:
            ...
            return Book.objects.filter(user__id=self.request.user.id, name__icontains=query)


        elif 'company' in self.request.GET:
            query = self.request.GET.get('company')
            return Book.objects.filter(user__id=self.request.user.id, company__icontains=query)

        elif 'number' in self.request.GET:
            query = self.request.GET.get('number')
            return Book.objects.filter(user__id=self.request.user.id, number__icontains=query)

        elif 'price' in self.request.GET:
            query = self.request.GET.get('price')
            return Book.objects.filter(user__id=self.request.user.id,price=query)
        else:
